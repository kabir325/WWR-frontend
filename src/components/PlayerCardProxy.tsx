...
export default function PlayerCardProxy({ piId, name, proxyUrl }: PlayerCardProps) {
  ...
  const fetchStatus = async () => {
    try {
      const response = await axios.get(`${proxyUrl}/api/${piId}/status`, {
        timeout: 5000,
      });
      
      if (response.data.success && response.data.data) {
        const statusData = response.data.data.status || response.data.data;
        setStatus(statusData);
        setVolume(Math.round((statusData.volume || 0.7) * 100));
        setError(null);
  ...
  const sendControl = async (action: string, data?: any) => {
    try {
      await axios.post(`${proxyUrl}/api/${piId}/control`, {
        action,
        ...data,
      });
